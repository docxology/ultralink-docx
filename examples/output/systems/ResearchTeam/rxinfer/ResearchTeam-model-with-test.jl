...
function test_ultralink_model()
    ...
    result = infer(
        model = model,
        data = (
            alice_chen = rand(Categorical([0.5, 0.5])),
            bob_smith = rand(Categorical([0.5, 0.5])),
            carol_jones = rand(Categorical([0.5, 0.5])),
        )
    )

    # Access posterior distributions
    posterior_alice_chen = result.posteriors[:alice_chen]
    posterior_bob_smith = result.posteriors[:bob_smith]
    posterior_carol_jones = result.posteriors[:carol_jones]
