# ultralink_model.jl
# Generated by UltraLink on 2025-03-10T17:42:20.335Z
...
function test_ultralink_model()
    ...
    result = infer(
        model = model,
        data = (
            camel = rand(Normal(0.0, 1.0)),
            fennec_fox = rand(Normal(0.0, 1.0)),
            desert_monitor = rand(Normal(0.0, 1.0)),
        )
    )

    # Access posterior distributions
    posterior_camel = result.posteriors[:camel]
    posterior_fennec_fox = result.posteriors[:fennec_fox]
    posterior_desert_monitor = result.posteriors[:desert_monitor]
